...
enum HelpContent {
    static let sections: [HelpSection] = [
        ...
        HelpSection(
            title: "Menu Rows",
            body: """
            Each tunnel row has an Active switch, Start at Login switch, Add Quick Forward button, and Check Now button. The row also shows the latest error when a start, check, or reconnect fails. The bottom action bar keeps two single-tap actions — Add Tunnel and Settings — with a More (…) menu for Copy Debug Log, How to Use, and Quit. State colors include Connected, Connecting, Reconnecting, Failed, and Idle (disconnected).
            """
        ),
        HelpSection(
            title: "Updates",
            body: """
            SSH Tunnel checks GitHub Releases for a newer version automatically — at most once every 24 hours. When a newer release is available, an Update available banner appears at the top of the menu and a notification is posted; both open the release page so you can download the new DMG and drag it to Applications. Because the app is not notarized, updates are installed manually and there is no silent self-update. Manual update checks and the automatic-check toggle live in the About & Updates popover, opened from the info button in the settings sidebar footer.
            """
        ),
        HelpSection(
            title: "Lifecycle",
